...
func (r *ReconcileImoocPod) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil{
		reqLogger.Error(err,"get existing pods error...")
		return reconcile.Result{}, err
	}
	//2)取到pod列表中的pod name
	//
	...
	//Deepcopy
	status := k8sv1alpha1.ImoocPodStatus{  //expected status
		PodNames: existingPodNames,
		Replicas: len(existingPodNames),
	}
	if !reflect.DeepEqual(instance.Status,status){
		instance.Status = status
		err = r.client.Status().Update(context.TODO(),instance)  //update pod
		if err != nil{
			reqLogger.Error(err,"update pod error...")
			return reconcile.Result{}, err
		}
	}
	...
	if len(existingPodNames) > instance.Spec.Replicas{
		//delete
		reqLogger.Info("deleting pod",existingPodNames,instance.Spec.Replicas)
		pod := existingPods.Items[0]
		err = r.client.Delete(context.TODO(),&pod)
		if err != nil{
			reqLogger.Error(err,"delete pod error...")
			return reconcile.Result{}, err
		}
	}
	...
	if len(existingPodNames) < instance.Spec.Replicas{
		//create
		reqLogger.Info("creating pod",existingPodNames,instance.Spec.Replicas)
